...
router.get (/^\/([a-zA-Z0-9\-]*)(\.html)?$/, function (req, res, next) {
	...
	if ( found.length < 3 || found [1] === '' ) // default to index.html
		return (renderPage ('index', { 'def': housedef, 'scenarie': scenario.scenarie () }, req, res)) ;
	fs.exists ('./views/' + found [1] + '.ejs', function (exists) {
		if ( exists )
			return (renderPage (found [1], { 'query': req.query, 'def': housedef, 'scenarie': scenario.scenarie () }, req, res)) ;
		next () ;
	}) ;
}) ;
